...
# - creates a cmake build target for the specified target. cmake target name: <target_name>_target.
# - creates a cmake test target. cmake target name: <target_name>_test.
# - creates a cmake test rapport target. cmake target name: <target_name>_test_report.
# - creates a cmake doc target. cmake target name: <target_name>_doc_target.
#
# Rust targets can be configured in 2 flavors, as executables (default) or as library.
#
# With passing the "LIB" flag instructs this function to build a library type. The actual native library type is set inside of the Cargo.toml of the corresponding
# crate. Couple of important notes!:
# - Static libraries only are supported at the moment.
# - C/C++ native dependencies are omitted in the produced library.
#
# Implementation note: cargo is not able to build specific libraries in isolation. this means that if 3 library builds are defined, cargo is called 3 times to build *all* libraries.
# As is looks is the additional build cost minimal. cargo defers building if nothing changed. Though it can introduce some unwanted side effects. think of build steps that write 
# and read generated files. the same happens for cargo test and doc targets.
#
# possible arguments:
# +-----------+----------------------------------------------------------------------+----------+
...
# | USED_BY   | add this target as a dependency for the specified cmake targets      | x        |
# +-----------+----------------------------------------------------------------------+----------+
function(cargo_build)
  cmake_parse_arguments(CARGO "LIB" "NAME;SOURCE_DIR" "EXT_LIB;USED_BY" "" ${ARGN})
  string(REPLACE "-" "_" LIB_NAME ${CARGO_NAME})

  # export linkage configuration
  setup_binding(${LIB_NAME} CUSTOM EXT_LIB ${CARGO_EXT_LIB})
  ...
  set(TARGET_OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/${LIB_TARGET}/${LIB_BUILD_TYPE}")
  set(LIB_FILE
  ...
  set(CARGO_ENV_COMMAND
    ...
    if(CARGO_LIB)
      add_custom_command(
        OUTPUT ${LIB_FILE}
        COMMAND ${CARGO_ENV_COMMAND} ${CARGO_EXECUTABLE} ARGS build ${CARGO_ARGS} ${TARGET_TYPE}
        COMMAND CMAKE_AR=${CMAKE_AR} CMAKE_RANLIB=${CMAKE_RANLIB} 
            ${PROJECT_SOURCE_DIR}/scripts/build/rust_strip_cxx_from_library.sh ${LIB_FILE}
        WORKING_DIRECTORY ${CARGO_SOURCE_DIR}
        DEPENDS ${LIB_SOURCES} ${CARGO_EXT_LIB}
        COMMENT "Running: cargo build ${CARGO_NAME}")
      add_custom_target(${CARGO_NAME}_target ALL DEPENDS ${LIB_FILE} ${LIB_SOURCES} ${CARGO_EXT_LIB})
      foreach(target ${CARGO_USED_BY})
        add_dependencies(${target} ${CARGO_NAME}_target)
        target_link_libraries(${target} PRIVATE ${LIB_FILE} Threads::Threads dl)
      endforeach()
    else()
      add_custom_target(${CARGO_NAME}_target)
        add_custom_command(
          TARGET ${CARGO_NAME}_target
          COMMAND ${CARGO_ENV_COMMAND} ${CARGO_EXECUTABLE} ARGS build ${CARGO_ARGS} ${TARGET_TYPE}
          WORKING_DIRECTORY ${CARGO_SOURCE_DIR}
          DEPENDS ${LIB_SOURCES} ${CARGO_EXT_LIB}
        COMMENT "Running: cargo build ${CARGO_NAME}")
    endif()
    add_dependencies(${CARGO_NAME}_target ${CARGO_EXT_LIB})
  ...
  add_custom_command(
    ...
    COMMAND ${CARGO_ENV_COMMAND} ${CARGO_EXECUTABLE} test ${CARGO_ARGS} ${TARGET_TYPE}
    WORKING_DIRECTORY ${CARGO_SOURCE_DIR}
    DEPENDS ${LIB_SOURCES} ${CARGO_EXT_LIB}
  ...
  add_custom_command(
    ...
    COMMAND ${CARGO_ENV_COMMAND} ${CARGO_EXECUTABLE} test ${CARGO_ARGS} ${TARGET_TYPE} -- -Z
            unstable-options --format json | cargo2junit > ${TEST_OUTPUT_FILE}
    WORKING_DIRECTORY ${CARGO_SOURCE_DIR}
    DEPENDS ${LIB_SOURCES} ${CARGO_EXT_LIB}
  ...
  add_custom_command(
    OUTPUT ${DOC_FILE}
    COMMAND ${CARGO_ENV_COMMAND} ${CARGO_EXECUTABLE} ARGS doc --no-deps ${CARGO_ARGS}
  ...
  if(NOT CARGO_LIB)
    # the cargo "deb" command expects binaries in the release output folder
    # to make it work for debug, copy the debug output to the release output
    #
    # deb are build with --no-buid, this is because if we let cargo deb build
    # it will try to release all cargo packages. Also non related crates
    add_custom_command(
      OUTPUT ${DEB_FILE}
      COMMAND LIB_BUILD_TYPE=${LIB_BUILD_TYPE} deb_release_dir=${CMAKE_CURRENT_BINARY_DIR}/${LIB_TARGET}/release binary_dir=${TARGET_OUTPUT}/${CARGO_NAME};
              ${PROJECT_SOURCE_DIR}/scripts/build/rust_cp_for_cargo_deb.sh
      COMMAND ${CARGO_ENV_COMMAND} ${CARGO_EXECUTABLE} ARGS deb 
              --no-strip --no-build --deb-version ${APP_VERSION} --output ${DEB_FILE} --target ${LIB_TARGET}
      WORKING_DIRECTORY ${CARGO_SOURCE_DIR}
      DEPENDS ${LIB_SOURCES}
    ...
    add_dependencies(${CARGO_NAME}_deb_target ${CARGO_NAME}_target)
  endif()
...
# This function exports build configuration of specified cmake targets. Its used for 2 Rust related tasks:
# - The rust compiler uses this data so it knows how to build against these native libraries.
# - Rust has a bindings generators that produces binding files based on this exported data
# see rust's project: uic_bindgen.
#
# These configuration files are .json files that end up in the CMAKE_BINARY_DIR. e.g. `my_configuration_bindings.json`
# uic_bindgen requires these files to be there in order to function.
# 
# usage:
# `setup_binding(my_component)` - basic declaration. my_component needs to be an valid cmake target.
#
# `setup_binding(my_component ALLOWLIST "my_component$" BLOCKLIST "my_block.*")` - configure the rust binding generator to create bindings
#   for vars, functions and types that end of name "my_components". Dont create bindings for vars, functions, types that start with "my_block"
#   ALLOWLIST and BLOCKLIST both accept regex in order to maniplutate of which items to create bindings for.
#
# `setup_binding(my_component EXT_LIB my_depedent_component)` - with EXT_LIB you can define adittional dependencies that are needed in order
#   to help the rust compiler to resolve dependant libraries. On default will `setup_binding` already export all dependencies of the specified target.
#
# `setup_binding(my_component CUSTOM)` - flagging this method with CUSTOM generates a configuration file of a non existing target. this can be useful
# in very specific scnario's. be aware that cmake cannot reslove any link libraries in this mode.
#
# possible arguments:
# +-----------+----------------------------------------------------------------------+----------+
# | Argument  |                             Description                              | optional |
...
function(setup_binding)
  ...
  cmake_parse_arguments(PARSED_ARGS "CUSTOM" "ALLOWLIST;BLOCKLIST" "EXT_LIB" ${ARGV} )

  set(json "{\"target_source_dir\": \"\"}")
  string(JSON json SET ${json} "name" "\"${TARGET}\"")
  string(JSON json SET ${json} "allow_list" "\"${PARSED_ARGS_ALLOWLIST}\"")
  string(JSON json SET ${json} "block_list" "\"${PARSED_ARGS_BLOCKLIST}\"")

  if (NOT PARSED_ARGS_CUSTOM)
    string(JSON json SET ${json} "target_source_dir" "\"$<TARGET_PROPERTY:${TARGET},SOURCE_DIR>\"")
  ...
  foreach(ll ${target_link_libraries})
    if (TARGET ${ll})
      ...
      if (target_type MATCHES "UTILITY" OR target_type MATCHES "INTERFACE_LIBRARY")
          continue()
      ...
      set(json_link_item "{ \"name\": \"${ll}\",\"typ\": \"${typ}\",\"search_path\": \"$<TARGET_FILE_DIR:${ll}>\"}")
  ...
  file(GENERATE OUTPUT ${uic_dir}/${TARGET}_bindgen.json CONTENT ${json})
  endfunction()
