...
armhf_cmake_params="-DCMAKE_C_COMPILER=/usr/bin/arm-linux-gnueabihf-gcc -DCMAKE_CXX_COMPILER=/usr/bin/arm-linux-gnueabihf-g++ -DPACK_DEBIAN_PACKAGE_ARCHITECTURE=armhf \
  -DCPACK_FILE_NAME_EXTRA=${os_version_codename} \
  -DPKG_CONFIG_EXECUTABLE=usr/bin/arm-linux-gnueabihf-pkg-config -DCMAKE_SYSTEM_NAME=Linux -DCMAKE_SYSTEM_PROCESSOR=armhf -DCONFIGURE_HOST=arm-linux"

arm64_cmake_params="-DCMAKE_C_COMPILER=/usr/bin/aarch64-linux-gnu-gcc -DCMAKE_CXX_COMPILER=/usr/bin/aarch64-linux-gnu-g++ -DPACK_DEBIAN_PACKAGE_ARCHITECTURE=arm64 \
  -DCPACK_FILE_NAME_EXTRA=${os_version_codename} \
  -DPKG_CONFIG_EXECUTABLE=usr/bin/aarch64-linux-gnu-pkg-config -DCMAKE_SYSTEM_NAME=Linux -DCMAKE_SYSTEM_PROCESSOR=arm64 -DCONFIGURE_HOST=arm64-linux"
