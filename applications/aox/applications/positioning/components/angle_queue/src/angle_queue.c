                             ...
                             aoa_angle_t *angle)
{
  ...
  if (SL_STATUS_OK != get_queue_by_id(tag_id, &queue)) {
    sl_log_error(LOG_TAG,
                 "angle_queue_push failed. "
                 "Could not get the queue by id for tag %s.",
                 tag_id);
 ...
 *****************************************************************************/
SL_WEAK void angle_queue_on_angles_ready(aoa_id_t tag_id,         
                                        uint32_t angle_count,           
                       ...
                       angle_queue_t *queue)
{
  ...
  for (int32_t i = angle_queue_config.sequence_ids - 1; i >= 0; i--) {
    if (queue->data[i].angle_count >= get_expected_angles(i)) {
        angle_queue_on_angles_ready(queue->tag_id,
