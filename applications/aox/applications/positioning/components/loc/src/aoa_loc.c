...
sl_status_t aoa_loc_finalize_config(void)
{
  ...
  // Add functionall locators to the estimator.
  while (NULL != current) {
    if (current->locator.functional) {
        ec = sl_rtl_loc_add_locator(&loc_libitem, &current->locator.item, &current->locator.loc_id);
        CHECK_ERROR(ec);
...
/**************************************************************************//**
 * Removes a locator from the list.
 *****************************************************************************/
