...
void test_init()
{
  ...
  TEST_ASSERT_EQUAL_MESSAGE(0, eed_config_init(), "eed_config_init failed");
  config_parse(sizeof(argv_inject) / sizeof(char *),
               argv_inject,
               "test version");
  eed_config_fixt_setup();
  TEST_ASSERT_EQUAL_STRING("localhost", eed_get_config()->mqtt_host);
  TEST_ASSERT_EQUAL(2000, eed_get_config()->mqtt_port);
}
