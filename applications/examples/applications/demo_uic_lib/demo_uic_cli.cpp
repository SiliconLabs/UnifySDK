...
// Unify demo application pre decleration of application commands functions
...
const std::map<std::string, std::pair<std::string, handler_func>> commands
  = {{"demo_command",
      {"Demo command doing nothing but adding a command to base CLI",
       handle_demo}}};
...
void demo_uic_cli_setup()
{
  ...
  uic_stdin_set_prompt_string("DEMO_Unify> ");
...
static sl_status_t handle_demo(const handle_args_t &arg)
{
  sl_log_debug(LOG_TAG, "Unify demo CLI printing args");
