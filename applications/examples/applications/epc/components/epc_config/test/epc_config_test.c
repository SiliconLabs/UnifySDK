...
void test_init()
{
  ...
  TEST_ASSERT_EQUAL_MESSAGE(0, epc_config_init(), "epc_config_init failed");
  config_parse(sizeof(argv_inject) / sizeof(char *),
               argv_inject,
               "test version");
  epc_config_fixt_setup();
  TEST_ASSERT_EQUAL_STRING("localhost", epc_get_config()->mqtt_host);
  TEST_ASSERT_EQUAL(2000, epc_get_config()->mqtt_port);
}
