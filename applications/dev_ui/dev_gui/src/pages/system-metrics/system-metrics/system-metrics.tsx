...
class SystemMetrics extends React.Component<SystemMetricsProps, SystemMetricsState> {
    ...
    getAreaChart = (dataField: string, valueName: string, title: string, yAxisProps: any, tooltipSign: string) => {
        if (this.state.SystemMetrics.Attributes[dataField] !== undefined) {
            return <Recharts.ResponsiveContainer height={235} className="col-sm-6 no-padding">
                <Recharts.AreaChart data={this.state.SystemMetrics.Attributes[dataField].map((i: number) => { return { [valueName]: i } })} margin={{ top: 25, right: 10, left: 25, bottom: 5 }}>
                    <defs>
    ...
    getRadialBarChart = (availableField: number, totalField: number, title: string, tooltipSign: string) => {
        if (availableField !== undefined && totalField !== undefined) {
            return <Recharts.ResponsiveContainer height={235} className="col-sm-4 no-padding">
    ...
    getBarChart = (data: any, tooltipSign: string) => {
        return <Recharts.ResponsiveContainer height={235} className="col-sm-3 no-padding">
                ...
                </Row>
                {
                    (!attrs || !this.props.IsConnected)
                        ...
                        <div className="col-sm-12 cluster-attributes">
                            <Card hidden={attrs.CPUUsagePercent === undefined}>
                                ...
                                <Card.Body className='inline no-padding'>
                                    ...
                                    {!cpuData?.length
                                        ? <></>
                                        : <Recharts.ResponsiveContainer height={235} className="col-sm-12 no-padding">
                                            <Recharts.ComposedChart data={cpuData} margin={{ top: 25, right: 10, left: 25, bottom: 5 }}>
                                                <Recharts.Legend />
                            ...
                            <br />
                            <Card hidden={attrs.RAMTotalMB === undefined}>
                            ...
                            {attrs.DisksUsage?.map((disk: any, index: number) => {
                                return <Fragment key={index}> <Card>
                            ...
                            <Card hidden={!attrs.WirelessNetworkInterfacesData?.length}>
                            ...
                            <br />
                            <Card hidden={attrs?.BatteryPercentage === undefined && attrs?.CurrentTemperatureCelcius === undefined && attrs?.SystemInterrupts === undefined && attrs?.UptimeMinutes === undefined}>
                                <Card.Body>
