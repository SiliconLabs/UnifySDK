                                          ...
                                          int8_t rssi_value)
{
  ...
  //Notify the TX module that we received something
  zwave_tx_on_frame_received(&connection_info,
                             &rx_options,
                                            ...
                                            zwave_home_id_t nwi_home_id)
{
  ...
  switch (status) {
    ...
    case UPDATE_STATE_NODE_INFO_SMARTSTART_HOMEID_RECEIVED_LR:  // SmartStart Prime received on Long Range
      ...
      // Notify the Z-Wave controller, which will dispatch the event further.
      zwave_controller_on_smart_start_inclusion_request(nwi_home_id,
                                                        already_included,
                                                        &received_nif,
...
/**
 * @brief function that converts the data received from the \ref ZWAPI
