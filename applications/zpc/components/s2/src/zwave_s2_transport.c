...
static bool zwave_tx_use_parent_frame_options           = false;
...
void S2_send_done_event(struct S2 *ctxt, s2_tx_status_t status)
{
  ...
  zwave_tx_use_parent_frame_options = false;
                      ...
                      uint16_t len)
{
  ...
  options.use_parent_frame_options = zwave_tx_use_parent_frame_options;
  options.valid_parent_session_id  = zwave_tx_valid_parent_session_id;
  options.parent_session_id        = zwave_tx_parent_session_id;

  // One exception, do not use the parent frame options
  // for Nonce Get(s)
  if (len >= 2 && buf[1] == SECURITY_2_NONCE_GET) {
    options.use_parent_frame_options = false;
  }
                            ...
                            uint16_t len)
{
  ...
  // Only the original multicast frame will trigger some follow-ups.
  options.group_id = conn->r_node;
  // Use the parent frame options.
  options.use_parent_frame_options = true;
  options.valid_parent_session_id  = zwave_tx_valid_parent_session_id;
  options.parent_session_id        = zwave_tx_parent_session_id;
                     ...
                     zwave_tx_session_id_t parent_session_id)
{
  ...
  if (connection->remote.is_multicast == false) {
    s2_connection.tx_options
      = tx_options->number_of_responses > 0 ? S2_TXOPTION_VERIFY_DELIVERY : 0;
  }
  ...
  zwave_tx_use_parent_frame_options = true;
