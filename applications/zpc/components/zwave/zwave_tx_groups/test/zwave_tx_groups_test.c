...
void test_zwave_tx_groups_manual_additions()
{
  // Create a group with an initial set of nodes
  zwave_nodemask_t my_test_nodemask;
  memset(my_test_nodemask, 0, sizeof(zwave_nodemask_t));

  // Force add 2 nodes into a group.
  zwave_multicast_group_id_t test_group = 25;
  zwave_node_id_t node_id               = 101;
  ZW_ADD_NODE_TO_MASK(node_id, my_test_nodemask);

  TEST_ASSERT_EQUAL(SL_STATUS_OK,
                    zwave_tx_add_node_to_group(node_id, test_group));
  ...
  // Adding the same again should do nothing
  TEST_ASSERT_EQUAL(SL_STATUS_OK,
                    zwave_tx_add_node_to_group(node_id, test_group));

  // Now I expect that my test_group is returned for my node_mask
  zwave_multicast_group_id_t received_group = ZWAVE_TX_INVALID_GROUP;
  TEST_ASSERT_EQUAL(SL_STATUS_OK,
                    zwave_tx_assign_group(my_test_nodemask, &received_group));

  // We expect group test_group is returned for my node_mask.
  TEST_ASSERT_EQUAL(test_group, received_group);

  // Lock our test group
  TEST_ASSERT_FALSE(zwave_tx_is_group_locked(test_group));
  TEST_ASSERT_EQUAL(SL_STATUS_OK, zwave_tx_lock_group(test_group));
  TEST_ASSERT_TRUE(zwave_tx_is_group_locked(test_group));

  // Try to force add one more node:
  TEST_ASSERT_EQUAL(SL_STATUS_FAIL,
                    zwave_tx_add_node_to_group(node_id, test_group));
}
