...
uint16_t get_theoretical_max_fragment_size(attribute_store_node_t endpoint_node)
{
  ...
  uint16_t maximum_fragment_size
    = zwave_tx_scheme_get_max_payload(node_id, endpoint_id);
...
{
  uint32_t node_supports_activation = 0;
  ...
  attribute_store_get_reported(activation_node,
                               &node_supports_activation,
                               sizeof(node_supports_activation));

  /// FIXME: Decide if we want to report true from v5 (like right now, where we don't check the version)
  /// (where nodes may not do the activation) or from v7, where nodes with do it with certainty

  return (node_supports_activation > 0);
}
  ...
  const char *file_name)
{
  ...
  if (node_supports_activation(endpoint_node) == false && time_difference > 0) {
