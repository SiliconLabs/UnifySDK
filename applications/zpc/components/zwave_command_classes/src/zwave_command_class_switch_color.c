...
// Apply a function for all child nodes of a type for the color switch attributes
#define COLOR_SWITCH_INVOKE_ON_ALL_ATTRIBUTES(state_node,           \
                                              child_node_type,      \
                                              function)             \
  {                                                                 \
    uint32_t index = 0;                                             \
                                           ...
                                           zpc_resolver_event_t event)
{
  ...
  switch (event) {
    case FRAME_SENT_EVENT_OK_SUPERVISION_WORKING:
      COLOR_SWITCH_INVOKE_ON_ALL_ATTRIBUTES(
        state_node,
        ATTRIBUTE_COMMAND_CLASS_SWITCH_COLOR_DURATION,
