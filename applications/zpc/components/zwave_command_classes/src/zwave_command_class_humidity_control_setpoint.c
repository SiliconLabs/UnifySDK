                                 ...
                                 humidity_control_setpoint_value_t value)
{
  ...
  sl_status_t status = attribute_store_get_child_reported(setpoint_node,
                                                          ATTRIBUTE(MIN_VALUE),
                                                          &min_value,
                                                          sizeof(min_value));
  ...
  status = attribute_store_get_child_reported(setpoint_node,
                                              ATTRIBUTE(MAX_VALUE),
                                              &max_value,
                                              sizeof(max_value));
  ...
  if (!result) {
    sl_log_error(LOG_TAG, "Value is out of bounds");
  }
  ...
  uint8_t start_index)
{
  ...
  humidity_control_setpoint_value_t value
    = command_class_get_int32_value(size,
                                    precision,
                                    &frame_data[current_index]);

  // Check bounds
  if (value_type == SETPOINT_CURRENT_VALUE
      && !check_setpoint_value_bounds(setpoint_node, value)) {
    return 0;
  }
...
static void zwave_command_class_humidity_control_setpoint_on_scale_attribute_update(
    attribute_store_node_t updated_node, attribute_store_change_t change) {
  if (change == ATTRIBUTE_DELETED) {
    return;
  }
  ...
  attribute_store_node_t node, uint8_t *frame, uint16_t *frame_length)
{
  ...
  if (!check_setpoint_value_bounds(setpoint_node, desired_value)) {
    return SL_STATUS_NOT_SUPPORTED;
  }
  ...
  humidity_control_setpoint_scale_t scale;
  status = attribute_store_get_child_reported(node,
                                              ATTRIBUTE(VALUE_SCALE),
                                              &scale,
                                              sizeof(scale));

  if (status != SL_STATUS_OK) {
    sl_log_error(LOG_TAG, "Can't get scale for SET.");
    return SL_STATUS_NOT_SUPPORTED;
  }
