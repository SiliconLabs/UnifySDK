...
static void on_remove_node_status_update(LEARN_INFO *remove_node_information)
{
  switch (remove_node_information->bStatus) {
    ...
    case REMOVE_NODE_STATUS_REMOVING_CONTROLLER:
      ...
      nms.tmp_node = remove_node_information->bSource;
      sl_log_debug(LOG_TAG,
    ...
    case REMOVE_NODE_STATUS_DONE:
      sl_log_debug(LOG_TAG, "Remove Node operation completed.\n");
      process_post(&zwave_network_management_process,
...
{
  zwapi_memory_get_ids(&nms.cached_home_id, &nms.cached_node_id);
}
...
static void on_learn_mode_callback(LEARN_INFO *learn_mode_info)
{
  switch (learn_mode_info->bStatus) {
    case LEARN_MODE_STARTED:
      ...
      nms.tmp_node = learn_mode_info->bSource;
      break;
    ...
    case NM_IDLE:
      ...
      if (ev == NM_EV_SET_DEFAULT) {
        ...
        if (!nms.smart_start_enabled) {
          // If smart start is disabled then we stop the smart start mode
        ...
        sl_log_info(LOG_TAG, "Starting inclusion of a Z-Wave node.\n");
        network_management_refresh_cached_node_list();
...
//      }
    ...
    case NM_REPLACE_FAILED_REQ:
      if(ev ==NM_EV_TIMEOUT || ev == NM_EV_REPLACE_FAILED_STOP || ev == NM_EV_REPLACE_FAILED_FAIL) {
        ...
        if(suc_node != MyNodeID &&  SupportsCmdClass(suc_node, COMMAND_CLASS_INCLUSION_CONTROLLER)) {
           rNM_EV_ADD_NODE_FOUNDd_register_new_node(nms.tmp_node, RD_NODE_FLAG_JUST_ADDED);
        ...
        if(common_flags & NODE_FLAG_SECURITY0) {
          if(nms.flags & NMS_FLAG_PROXY_INCLUSION ) {
            ...
            security_add_begin(nms.tmp_node, nms.tx_options,
    ...
    case NM_SEND_NOP:
      if (ev == NM_EV_TIMEOUT) {
        sl_log_info(LOG_TAG,
                    "Initiating Self destruct verification NodeID %d\n",
                    nms.tmp_node);
      ...
      connection_info.remote.node_id                     = nms.tmp_node;
    ...
    case NM_FAILED_NODE_REMOVE:
      if (ev == NM_EV_NOP_FAIL) {
        if (nms.state == NM_WAIT_FOR_TX_TO_SELF_DESTRUCT) {
          ...
          sl_log_debug(LOG_TAG, "Self destruct nodeid %u\n", nms.tmp_node);
          etimer_set(&nms.timer, 20000); /* TODO: Is 20 sec. reasonable? */
        } else if (nms.state == NM_FAILED_NODE_REMOVE) {
          nms.state = NM_WAITING_FOR_FAILED_NODE_REMOVAL;
        }
        if (zwapi_remove_failed_node(nms.tmp_node,
    ...
    case NM_NODE_FOUND:
      ...
      if (ev == NM_EV_ADD_CONTROLLER || ev == NM_EV_ADD_END_NODE) {
        ...
        if ((nms.tmp_node != 0)
            && (ZW_IS_NODE_IN_MASK(nms.tmp_node, nms.cached_node_list))
                 == false) {
          zwave_controller_on_node_id_assigned(nms.tmp_node, true);
        }
    ...
    case NM_WAIT_FOR_PROTOCOL:
      if (ev == NM_EV_ADD_PROTOCOL_DONE) {
        ...
        if ((nms.tmp_node != 0)
            && (ZW_IS_NODE_IN_MASK(nms.tmp_node, nms.cached_node_list) == true)
        ...
        zwapi_get_protocol_info(nms.tmp_node, &ni);
        ...
        if (nms.flags & NMS_FLAG_SMART_START_INCLUSION) {
          nms.state = NM_WAIT_FOR_SECURE_ADD;
          zwave_s2_start_add_node(nms.tmp_node);
          zwave_security_validation_set_node_as_s2_capable(nms.tmp_node);
          ...
          zwave_s2_start_add_node(nms.tmp_node);
                   ...
                   == SL_STATUS_OK) {
          nms.state = NM_WAIT_FOR_SECURE_ADD;
          zwave_s0_start_bootstrapping(nms.tmp_node, nms.is_controller);
        } else {
    ...
    case NM_WAIT_FOR_SECURE_ADD:
      if (ev == NM_EV_SECURITY_DONE) {
        ...
        if (nms.flags & NMS_FLAG_SMART_START_INCLUSION) {
          ...
          // Non-smart start inclusions sends the request upwards
    ...
    case NM_WAITING_FOR_FAILED_NODE_REMOVAL:
      if (ev == NM_EV_REMOVE_FAILED_OK) {
        sl_log_debug(LOG_TAG,
                     ...
                     "successful!\n");
        network_management_refresh_cached_node_list();
        zwave_controller_on_node_deleted(nms.tmp_node);
        nms.state = NM_IDLE;
    ...
    case NM_LEARN_MODE:
      if (ev == NM_EV_TIMEOUT) {
        ...
        if ((nms.flags & (NMS_FLAG_LEARNMODE_NWI | NMS_FLAG_LEARNMODE_NWE))
            && (nms.count < 4)) {
          if (nms.flags & NMS_FLAG_LEARNMODE_NWI) {
            sl_log_info(LOG_TAG, "Sending explore_request_inclusion\n");
            zwapi_explore_request_inclusion();
          } else {
            sl_log_info(LOG_TAG, "Sending explore_request_exclusion\n");
