...
typedef struct {
  ...
  /// This callback is fired when a frame has beed received and
  /// de-encapsulated.
  ///
  ...
  /// Frame Transmission to given node is failed
  /// @param node_id NodeID for which frame transmission failed
  void (*on_frame_transmission_failed)(zwave_node_id_t node_id);
  /// Successful Frame Transmission to given node
  /// @param node_id NodeID for which frame transmission is successful
  void (*on_frame_transmission_success)(zwave_node_id_t node_id);
