...
#    return to_upper_camel_case(ns+name)+"Bits"
  ...
  if(param.tag == 'param' ):
    raise Exception('Type param::%s Not implemented ' % param.attrib['type'])
  else:
...
def generate_marker_generator(parent):
  ...
  for p in parent:
    if( p.tag in ['const']):
      x = x + 'frame.append( FrameParser::from(%s as u8) );\n' % p.attrib['flagmask']
...
def generate_struct_byte_generator(parent):
  ...
  for p in parent:
    ...
    if(p.tag in ['bitfield','fieldenum'] ):
      ...
      x = x + '    _byte = _byte | ((u8::from(_typ.%s) << %s) & %s);\n' % (name,shift,mask)
    if(p.tag == 'bitflag'):
...
def generate_marker_parser(parent):
  ...
  for p in parent:
    if( p.tag in ['const']):
      x = x + 'if _frame.read() != %s { return Err(FrameParserError{}) }\n' % p.attrib['flagmask']
  x = x + 'Ok( %s{} )' % maker_name
...
def generate_struct_parser(parent):
  ...
  for p in parent:
    ...
    if( p.tag == 'param' and p.attrib['type'] == 'VARIANT'):
      ...
      x = x +   '    let %s = %s::try_from(&mut frame2)?;\n' % (name,type_name)
    elif( p.tag in ['variant_group']):
      x = x +   '    let %s_len = _frame.read_label(%s,%s);\n' % (name,p.attrib['sizeoffs'],p.attrib['sizemask'])
      x = x +   '    let mut %s = Vec::new();\n' % name
      x = x +   '    for _i in 0..%s_len {\n' % name
      x = x +   '      %s.push( %s::try_from(&mut frame2)? );\n' % (name,type_name)
      x = x +   '    };\n'
...
''' % ( type_of_param(cc) )
  x = x  + '    if %s != frame.read() { return Err(FrameParserError{}) } ; \n' % (cc.attrib['key'])
...
def generate_cmd(cmd):
  ...
  x = x + foreach_xml( cmd, 'param','STRUCT_BYTE', generate_struct_byte_generator)
  ...
  x = x + foreach_xml( cmd, 'param','STRUCT_BYTE', generate_struct_byte_parser)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Make command class parsers for rust.')
  parser.add_argument('--xml',required=True, help='Z-Wave command class xml file')
  parser.add_argument('--outdir',required=True, help='directory to write command pasers to')
