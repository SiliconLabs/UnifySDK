...
use crate::{
    zwave_command_class_send_report, SL_STATUS_ALREADY_EXISTS, SL_STATUS_FAIL,
    SL_STATUS_IS_WAITING, SL_STATUS_NOT_SUPPORTED, SL_STATUS_OK,
};
use ::zwave_tx::*;
use crc16::*;
...
const LOG_TAG: &str = "zwave_cc_fwu";
        ...
        *length = 0;
        ...
        match desired_transfer_value {
            ...
            ZwaveFirmwareTransferState::WAITING_FOR_ACTIVATION => {
                ...
                let manufacturer_id_node: uic_attribute_store::attribute_store_node_t =
                    attribute_store_get_node_child_by_type(
                        fwu_node,
                ...
                let firmware_target_node: uic_attribute_store::attribute_store_node_t =
                    attribute_store_get_node_child_by_type(
                        node,
                ...
                let fwu_fw_node: uic_attribute_store::attribute_store_node_t =
                    attribute_store_get_node_child_by_type(
                        fwu_node,
                        ATTRIBUTE_COMMAND_CLASS_FWU_MD_FWU_FW,
                        firmware_target as u32,
                    );
                let firmware_id_node: uic_attribute_store::attribute_store_node_t =
                    attribute_store_get_node_child_by_type(
                        fwu_fw_node,
                ...
                let transfer_filename_node: uic_attribute_store::attribute_store_node_t =
                    attribute_store_get_node_child_by_type(
                        node,
...
fn handle_fwu_md_get(
    ...
    match attribute_store::read_attribute(
        ...
        Ok(v) => fragment_size = v,
        Err(status) => {
            log_warning(LOG_TAG, "Reading FWU fragment size failed.".to_string());
            zwave_firmware_update_send_abort_frame(
                fwu_node_info.ep_node,
                &fwu_node_info.info,
                get_frame.report_number,
    ...
    match attribute_store::read_attribute(
        ...
        Err(status) => {
            ...
            log_warning(
                LOG_TAG,
                "Reading filename from attribute store failed.".to_string(),
            );
            zwave_firmware_update_send_abort_frame(
                fwu_node_info.ep_node,
                &fwu_node_info.info,
...
fn handle_fwu_md_prepare_get(
    ...
    if uic_config::config_get_as_int("zpc.manufacturer_id")
    ...
    if uic_config::config_get_as_int("zpc.hardware_version")
...
fn handle_fwu_md_request_get(
        ...
        != request_get.manufacturer_id
    {
        ...
        && (uic_config::config_get_as_int("zpc.hardware_version")
            .unwrap_or(DEFAULT_HARDWARE_VERSION.into()) as u16
...
fn handle_fwu_md_request_report(
    ...
    let last_status = match request_report.status {
        0x00 => {
            log_info(LOG_TAG, "Invalid combination of Manufacturer ID and Firmware ID. Firmware update request declined.".to_string());
            ZwaveFirmwareTransferStatus::ABORTED
        }
        0x01 => {
            log_info(LOG_TAG, "Device expected an authentication event to enable firmware update. Firmware update request declined.".to_string());
            ZwaveFirmwareTransferStatus::NOT_AUTHORIZED
        }
        0x02 => {
            log_error(LOG_TAG, "The requested fragment size exceeds the max fragment size. Firmware update request declined.".to_string());
            ZwaveFirmwareTransferStatus::ABORTED
        }
        0x03 => {
            log_info(
                    LOG_TAG,
                    format!(
                        "{} reports that its firmware target is not upgradable. Firmware update request declined.",
                        fwu_node_info
                    ),
                );
        ...
        0x04 => {
            log_warning(
                LOG_TAG,
                "Invalid hardware version. Firmware update request declined.".to_string(),
            );
            ZwaveFirmwareTransferStatus::ABORTED
        }
        0x05 => {
            log_info(LOG_TAG, "Another firmware image is currently being transferred. Firmware update request declined.".to_string());
            ZwaveFirmwareTransferStatus::ABORTED
        }
        0x06 => {
            log_info(
                LOG_TAG,
                "Insufficient battery level. Firmware update request declined.".to_string(),
            );
            ZwaveFirmwareTransferStatus::ABORTED
        }
        0xFF => {
            log_debug(
                LOG_TAG,
                "The node accepted to initiate the Firmware Update operation".to_string(),
            );
...
fn handle_fwu_activation_status_report(
    ...
    let last_status = match status_report.firmware_update_status {
        0x0 => {
            log_warning(LOG_TAG, "Invalid combination of manufacturer ID, firmware ID and hardware version or firmware target. The received image will not be stored.".to_string());
            ZwaveFirmwareTransferStatus::ABORTED
        }
        0x1 => {
            log_info(
        ...
        _ => {
            log_info(LOG_TAG,format!("Unknown status for Firmware Update Activation Report: {}. Assuming that the activation failed.",status_report.firmware_update_status));
            ZwaveFirmwareTransferStatus::ABORTED
        }
    };
    ...
    if unsafe {
        ...
        match attribute_store::read_attribute(
            ...
            Err(_) => {
                log_warning(
                    LOG_TAG,
                    "Error on reading FWU_TRANSFER desired value".to_string(),
                );
                return;
            }
        }
    ...
    match status_report.status {
        0xff => {
            ...
            if status_report.wait_time == 0 {
                ...
                let expiry_node: uic_attribute_store::attribute_store_node_t = unsafe {
                    attribute_store_get_node_child_by_type(
                        transfer_node,
                ...
                log_debug(LOG_TAG, format!("New firmware was successfully stored in temporary non volatile memory. The device may have restarted itself. Wait for {} seconds for the deivce to be ready.", status_report.wait_time));
            }
        }
        0xfe => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
        ...
        0xfd => {
            ...
            log_debug(LOG_TAG, "Firmware image downloaded successfully. The node now awaits for the activation command.".to_string());
        }
        0x0 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_warning(
                    LOG_TAG,
                    "The device was unable to receive the requested firmware data without checksum error.".to_string(),
                );
        }
        0x1 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_info(
                LOG_TAG,
                "The device was unable to receive the Firmware image..".to_string(),
            );
        }
        0x2 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_warning(
                    LOG_TAG,
                    "Supporting node reports invalid Manufacturer ID for the transfered image. Firmware Update failed.".to_string(),
                );
        }
        0x3 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_warning(
                  LOG_TAG,
                  "Supporting node reports invalid Firmware ID for the transfered image. Firmware Update failed.".to_string(),
              );
        }
        0x4 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_warning(
                LOG_TAG,
                "Supporting node reports invalid Firmware Target for the transfered image. Firmware Update failed.".to_string(),
              );
        }
        0x5 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_warning(
              LOG_TAG,
              "Supporting node reports invalid header information for the transfered image. Firmware Update failed.".to_string(),
                );
        }
        0x6 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_warning(
            LOG_TAG,
            "Supporting node reports invalid header format for the transfered image. Firmware Update failed.".to_string(),
                );
        }
        0x7 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_warning(
          LOG_TAG,
          "Supporting node reports insufficient memory for the transfered image. Firmware Update failed.".to_string(),
                );
        }
        0x8 => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
            ...
            log_warning(
        LOG_TAG,
        "Supporting node reports non-matching hardware version for the transfered image. Firmware Update failed.".to_string(),
              );
        }
        _ => {
            current_transfer_state = ZwaveFirmwareTransferState::IDLE;
...
fn handle_fw_md_get(fwu_node_info: FirmwareNodeInfo) -> sl_status_t {
    let report_frame = ZwaveFirmwareMdReportType {
        manufacturer_id: uic_config::config_get_as_int("zpc.manufacturer_id")
            .unwrap_or(DEFAULT_MANUFACTURER_ID.into()) as u16,
        ...
        firmware_n_id: vec![uic_config::config_get_as_int("zpc.hardware_version")
            .unwrap_or(DEFAULT_HARDWARE_VERSION.into()) as u16],
        hardware_version: uic_config::config_get_as_int("zpc.hardware_version")
...
#[generate_extern_c]
fn on_fwu_waiting_timer(node: u32, change: attribute_store_change_t) {
    let mut desired_fwu_value: ZwaveFirmwareTransferState = ZwaveFirmwareTransferState::IDLE;
    if unsafe {
        attribute_store_is_value_defined(
            node,
        ...
        && desired_fwu_value == ZwaveFirmwareTransferState::IDLE
    {
        ...
        match SystemTime::now().duration_since(SystemTime::UNIX_EPOCH) {
            ...
            Err(_) => log_warning(LOG_TAG, "SystemTime before UNIX EPOCH!".to_string()),
        }
        ...
        && desired_fwu_value == ZwaveFirmwareTransferState::IDLE
    {
        ...
        match attribute_store::read_attribute(
            ...
            Ok(v) => expiry_time = v,
            Err(status) => {
                log_info(LOG_TAG, "Error on reading expiry time".to_string());
                return;
            }
        }
        log_info(
            LOG_TAG,
            format!(
                "Device is rebooting after firmware update. Scheduling of reboot timer callback for attribute node: {:#04}",
                node
            ),
        );
        if let Err(e) = attribute_store::set_callback_on_timeout(
...
impl FirmwareNodeInfo {
    fn new(info: zwave_controller::zwave_controller_connection_info) -> Self {
        unsafe {
