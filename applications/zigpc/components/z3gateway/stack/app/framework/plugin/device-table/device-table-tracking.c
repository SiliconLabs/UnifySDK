...
EmberStatus emberAfPluginDeviceTableSendLeave(uint16_t index)
{
  ...
  if ((index == EMBER_AF_PLUGIN_DEVICE_TABLE_NULL_INDEX) || (index >= EMBER_AF_PLUGIN_DEVICE_TABLE_DEVICE_TABLE_SIZE) ) {
    emberAfCorePrintln("Device Table: Failed to find device to remove");
    status = EMBER_NOT_FOUND;
  }

  if (status == EMBER_SUCCESS) {
    nodeId = deviceTable[index].nodeId;
    // make sure this is a valid device table entry
    if (nodeId == EMBER_AF_PLUGIN_DEVICE_TABLE_NULL_NODE_ID) {
      emberAfCorePrintln("Device Table: Failed to find Node ID of device to remove");
      status = EMBER_NOT_FOUND;
    }
  }
