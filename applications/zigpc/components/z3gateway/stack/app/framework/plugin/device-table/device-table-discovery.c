...
/**
 * @brief Get the task queue entry index From EUI64
 *
 * @param eui64     EUI64 of taskEntry to find
 * @return uint8_t index of the matched task queue entry, or
 * DEVICE_QUEUE_NULL if not
 */
static uint8_t getTaskQueueIndexFromEUI64(const EmberEUI64 eui64)
{
  uint8_t taskIndex;

  if ((taskQueue == NULL) || (queueSize == 0)) {
    return DEVICE_QUEUE_NULL;
  }
  for (taskIndex = 0; taskIndex < queueSize; taskIndex++) {
    if (!MEMCOMPARE(taskQueue[taskIndex].eui64, eui64, EUI64_SIZE)) {
      return taskIndex;
    }
  }
  return DEVICE_QUEUE_NULL;
  ...
  if ((currentEntryPtr->delay == 0) && (queueSize > 0)) {
    ...
    switch (currentEntryPtr->state) {
      case DEVICE_DISCOVERY_STATE_ENDPOINTS_SEND:
        ...
        emberActiveEndpointsRequest(currentEntryPtr->nodeId,
                                             APS_OPTION_DISCOVER);
                                                  ...
                                                  uint16_t length)
{
  ...
  if (i != DEVICE_QUEUE_NULL) {
    ...
    entry.delay = 0;
    entry.waitTime = 0;
    endpointCount = message[ACTIVE_ENDPOINT_RESPONSE_COUNT_OFFSET];
    emberAfCorePrintln("number of ep: %d", endpointCount);
    emberAfPluginDeviceTableActiveEndpointsCallback(entry.eui64, endpointCount);
...
/**
 * @brief Process Device joins to the network. This function is called when:
 * 1. a new device has joined.
 * 2. device nodeID has changed.
 *
 * @param newNodeId
 * @param newNodeEui64
 */
                                            ...
                                            EmberEUI64 newNodeEui64)
{
  ...
  if (deviceTableIndex == EMBER_AF_PLUGIN_DEVICE_TABLE_NULL_INDEX) {
    ...
    /* Delay retrying for the maximum wait time for now */
    entry.delay = DEVICE_DISCOVERY_MAX_WAIT_TICKS;
    entry.waitTime = 0;
    MEMCOPY(entry.eui64, newNodeEui64, EUI64_SIZE);
    addQueueEntry(taskQueue, &entry);
#ifndef EMBER_AF_PLUGIN_DEVICE_TABLE_MANUAL_SEND_ENDPOINTS_REQUEST
  emAfDeviceTableStartDeviceDiscovery(entry.eui64);
#endif /* EMBER_AF_PLUGIN_DEVICE_TABLE_MANUAL_SEND_ENDPOINTS_REQUEST */
  } else {
...
EmberStatus emAfDeviceTableStartDeviceDiscovery(const EmberEUI64 eui64)
{
  ...
  uint8_t deviceTaskIndex;
  DeviceTableQueueEntry *deviceDiscTask;


  deviceTaskIndex = getTaskQueueIndexFromEUI64(eui64);
  if (deviceTaskIndex == DEVICE_QUEUE_NULL) {
    status = EMBER_NOT_FOUND;
  }

  if(status == EMBER_SUCCESS) {
    deviceDiscTask = &taskQueue[deviceTaskIndex];
    /* Clear any delay set to run this task
     * (See emberAfDeviceTableNewDeviceJoinHandler)
     */
    deviceDiscTask->delay = 0;
  }
  ...
  if(decision != EMBER_DENY_JOIN) {
    switch (status) {
      case EMBER_STANDARD_SECURITY_UNSECURED_JOIN:
        ...
        emberAfPluginDeviceTableTrustCenterJoinCallback(newNodeEui64);
