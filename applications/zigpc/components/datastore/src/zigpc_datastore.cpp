...
zigbee_binding_t string_to_binding(const std::string &binding_str)
{
    ...
    binding.source_address = std::stoull(source_addr_string);
    ...
    binding.dest_address = std::stoull(dest_addr_string);
                ...
                size_t binding_list_size)
{
    ...
    auto endpoint_parent = ATTRIBUTE_STORE_INVALID_NODE;
    size_t count        = 0U;
    EndpointAccessor endpoint;
    ...
    count = endpoint.child_count(   endpoint_parent,
                                    endpoint_id,
                                    ZIGPC_DS_TYPE_BINDING);
  }

  if (count > binding_list_size) 
  {
        status = SL_STATUS_INVALID_COUNT;
  } 
  else if (count > 0) 
  {
        attribute_store_node_t parent = endpoint.find(endpoint_parent, endpoint_id);
        BindingListAccessor bindings;

        std::string string_list[count]; //NOSONAR - need c array to interact with binding accessor 

        status = bindings.read_list(parent, string_list, binding_list_size);

        for(unsigned int i = 0; i<count; i++)
        {
            binding_list[i] = string_to_binding(string_list[i]);
        }
  }
                ...
                size_t binding_list_size)
{
  ...
  if (status == SL_STATUS_OK) {
    attribute_store_node_t bindings_parent
      = endpoint.find(endpoint_parent, endpoint_id);
    BindingListAccessor bindings;

    status = bindings.remove_list(bindings_parent);
    if (status == SL_STATUS_OK) {
        
        std::string string_list[binding_list_size]; //NOSONAR - need c array to interact with BindingListAccessor

        for(unsigned int i = 0; i < binding_list_size; i++)
        {
            string_list[i] = binding_to_string(binding_list[i]);
        }

      status = bindings.create_list(bindings_parent,
                                      string_list,
                                      binding_list_size);
    }
  }
