...
#define MOCK_DEV_1_EP1_GROUP1_NODE_ID 5
...
void helper_set_group_parents_overhead(zigbee_eui64_uint_t *eui64_i,
...
void test_zigpc_datastore_get_group_count_sanity(void)
{
  // ARRANGE
  size_t count;
  ...
  helper_set_group_parents_overhead(&eui64_i,
                                    &endpoint_id,
  ...
  // ACT
  count = zigpc_datastore_get_group_count(eui64, endpoint_id, true);

  // ASSERT
  TEST_ASSERT_EQUAL(3, count);
}

void test_zigpc_datastore_get_group_count_no_device(void)
{
  // ARRANGE
  size_t count;
...
void test_zigpc_datastore_group_find_by_index(void)
{
  // ARRANGE
  sl_status_t status;
  size_t index                        = 3;
  ...
  zigbee_group_id_t expected_group_id = 0x0030;
...
void test_zigpc_datastore_contains_group_true(void)
{
  ...
  zigbee_group_id_t group_id       = 0x1234;
...
void test_zigpc_datastore_contains_group_false(void)
{
  zigbee_eui64_t eui64             = "\x03\x55\xFF\x20\x34\x11\x4A\xBB";
  zigbee_eui64_uint_t eui64_i      = 0x0355FF2034114ABB;
  zigbee_endpoint_id_t endpoint_id = 6;
  zigbee_group_id_t group_id       = 0x0054;
...
void test_zigpc_datastore_group_create(void)
{
  // ARRANGE
  sl_status_t status;
  ...
  zigbee_group_id_t group_id       = 0x0013;
  ...
  attribute_store_add_node_ExpectAndReturn(ZIGPC_DS_TYPE_GROUP_REPORTED,
                                           MOCK_DEV_1_EP1_NODE_ID,
                                           MOCK_DEV_1_EP1_GROUP1_NODE_ID);
  ...
  // ACT
  status = zigpc_datastore_create_group(eui64, endpoint_id, true, group_id);

  // ASSERT
  TEST_ASSERT_EQUAL_HEX8(SL_STATUS_OK, status);
}

void test_zigpc_datastore_read_group(void)
{
  // ARRANGE
  sl_status_t status;
  zigbee_eui64_t eui64             = "\x44\x55\xFF\x29\x34\x11\x4A\xB5";
  zigbee_eui64_uint_t eui64_i      = 0x4455FF2934114AB5;
  zigbee_endpoint_id_t endpoint_id = 11;
  zigbee_group_id_t group_id       = 0x1324;
  zigpc_group_data_t data;
  zigpc_group_data_t expected_data = {
    .group_name = "Room",
  };
  ...
  // ACT
  status
    = zigpc_datastore_read_group(eui64, endpoint_id, true, group_id, &data);

  // ASSERT
  TEST_ASSERT_EQUAL_HEX8(SL_STATUS_OK, status);
  TEST_ASSERT_EQUAL_MEMORY(&expected_data, &data, sizeof(zigpc_group_data_t));
}

void test_zigpc_datastore_write_group(void)
{
  // ARRANGE
  sl_status_t status;
  ...
  zigbee_endpoint_id_t endpoint_id = 15;
  zigbee_group_id_t group_id       = 0xFE12;
  zigpc_group_data_t data          = {.group_name = "Universe"};
  ...
  // ACT
  status
    = zigpc_datastore_write_group(eui64, endpoint_id, true, group_id, &data);
...
void test_zigpc_datastore_cluster_delete(void)
{
  ...
  zigbee_eui64_uint_t eui64_i      = 0x4455FFEE34114AB5;
  zigbee_endpoint_id_t endpoint_id = 15;
  zigbee_group_id_t group_id       = 0x0262;
  ...
  attribute_store_delete_node_ExpectAndReturn(MOCK_DEV_1_EP1_GROUP1_NODE_ID,
                                              SL_STATUS_OK);

  // ACT
  status = zigpc_datastore_remove_group(eui64, endpoint_id, true, group_id);
