...
void helper_expect_child_counts_overhead(attribute_store_node_t parent_node,
...
void test_zigpc_datastore_get_command_list_count_sanity(void)
{
  // ARRANGE
  size_t count;
  ...
  helper_expect_child_counts_overhead(MOCK_DEV_1_EP1_CL1_NODE_ID,
                                      ZIGPC_DS_TYPE_CLUSTER_GENERATED_COMMAND,
                                      expected_count);

  // ACT
  count = zigpc_datastore_get_command_count(eui64,
                                            endpoint_id,
...
void test_zigpc_datastore_get_command_list_count_no_cluster(void)
{
  // ARRANGE
  size_t count;
...
/**
 * @brief Read command list sanity test
 *
 */
void test_zigpc_datastore_command_list_read(void)
{
  // ARRANGE
  sl_status_t status;
  ...
  zcl_command_id_t command_list[expected_count];
  zcl_command_id_t expected_command_list[expected_count];
  ...
  for (size_t i = 0; i < expected_count; i++) {
    expected_command_list[i] = 0xFF - i;
    attribute_store_get_node_child_by_type_ExpectAndReturn(
      MOCK_DEV_1_EP1_CL1_NODE_ID,
      ZIGPC_DS_TYPE_CLUSTER_GENERATED_COMMAND,
      i,
    ...
    attribute_store_get_node_attribute_value_ReturnMemThruPtr_value(
      (uint8_t *)(expected_command_list + i),
      sizeof(zcl_command_id_t));
  }

  // ACT
  size_t actual_count = expected_count;
  status              = zigpc_datastore_read_command_list(eui64,
                                             endpoint_id,
                                             ZCL_CLUSTER_SERVER_SIDE,
                                             cluster_id,
                                             ZCL_CLUSTER_GENERATED_COMMAND,
  ...
  TEST_ASSERT_EQUAL_UINT8_ARRAY(expected_command_list,
                                command_list,
                                expected_count);
}
/**
 * @brief Write command list sanity test
 *
 */
void test_zigpc_datastore_command_list_write(void)
{
  // ARRANGE
  sl_status_t status;
  ...
  for (size_t i = 0; i < expected_count; i++) {
    ...
    // expect calls to create command
