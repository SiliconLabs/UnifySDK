...
void test_zigpc_binding_parse_bind_cmd(void)
{
  const char test_topic[]
    = "ucl/by-unid/zb-0000000000000001/ep1/Binding/Commands/Bind";
  const char test_message[] = "{\"ClusterName\":\"OnOff\",\"DestinationUnid\":"
                              "\"zb-0000000000000002\",\"DestinationEp\": 1}";
  ...
  // ARRANGE
  zigbee_uint_to_eui64_ExpectAndReturn(test_src_eui_uint, NULL, SL_STATUS_OK);
  zigbee_uint_to_eui64_IgnoreArg_dest();

  zigbee_uint_to_eui64_ExpectAndReturn(test_dest_eui_uint, NULL, SL_STATUS_OK);
  zigbee_uint_to_eui64_IgnoreArg_dest();


  zigpc_datastore_is_device_contained_ExpectAndReturn(test_dest_eui_uint,true);

  zigpc_gateway_request_binding_ExpectAndReturn(test_src_eui,
                                                1,
                                                0x06,
                                                test_dest_eui,
                                                1,
  ...
  zigpc_binding_parse_bind_cmd(test_topic, test_message, message_length);
...
{
  const char test_topic[]
    = "ucl/by-unid/zb-0000000000000001/ep1/Binding/Commands/Unbind";
  const char test_message[] = "{\"ClusterName\":\"OnOff\",\"DestinationUnid\":"
                              "\"zb-0000000000000002\",\"DestinationEp\": 1}";
  const size_t message_length = strlen(test_message);
  ...
  zigpc_gateway_request_unbind_ExpectAndReturn(test_src_eui,
                                               1,
                                               0x06,
                                               test_dest_eui,
                                               1,
  ...
  zigpc_binding_parse_unbind_cmd(test_topic, test_message, message_length);
...
void test_zigpc_binding_parse_cmd_args(void)
{
  ...
  sl_status_t status = zigpc_binding_parse_cmd_args(test_topic,
                                                    test_message,
                                                    message_length,
                                                    &binding);
