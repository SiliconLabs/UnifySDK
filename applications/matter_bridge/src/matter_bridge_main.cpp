...
void init_ember_endpoints()
{
  PlatformMgr().ScheduleWork([](intptr_t) {
    ...
    emberAfEndpointEnableDisable(emberAfEndpointFromIndex(static_cast<uint16_t>(
...
int main(int argc, char *argv[])
{
  ...
  uic_fixt_setup_step_t uic_fixt_setup_steps_list[]
    = {{matter_bridge_config_fixt_setup, "Matter Bridge config fixture"},
       {NULL, "Terminator"}};
  ...
  opt.payload.rendezvousInformation.Set(RendezvousInformationFlag::kOnNetwork);
  opt.mWiFi   = false;
  opt.mThread = false;
  ...
  if (CHIP_NO_ERROR
      != InterfaceId::InterfaceNameToId(cfg->interface, opt.interfaceId)) {
  ...
  IdentifyClusterCommandHandler identify_cluster_commands_handler(
    node_state_monitor);
  ...
  BridgedDeviceBasicInfoAttributeAccess bridge_device_basic_handler(
    node_state_monitor);
