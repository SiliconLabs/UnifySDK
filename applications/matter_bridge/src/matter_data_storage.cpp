...
template<> bool matter_data_storage::persist_data(endpoint_mapping &key_value)
{
  if (!key_value.matter_endpoint.has_value()) {
    sl_log_info(LOG_TAG, "The matter endpoint value is not provided.");
    return false;
  }
  std::string key = std::string(key_value.unify_unid) + "-"
                    + std::to_string(key_value.unify_endpoint);
  if (chip::DeviceLayer::PersistedStorage::KeyValueStoreMgrImpl().Put(
        key.c_str(),
        key_value.matter_endpoint.value())
      == CHIP_NO_ERROR) {
    return true;
  }
  return false;
...
template<> bool matter_data_storage::persist_data(group_mapping &key_value)
{
  ...
  if (chip::DeviceLayer::PersistedStorage::KeyValueStoreMgrImpl().Put(
        key.c_str(),
        key_value.unify_group_id.value())
      == CHIP_NO_ERROR) {
    return true;
  }
  return false;
...
bool matter_data_storage::get_persisted_data(endpoint_mapping &key_value)
{
  ...
  if (chip::DeviceLayer::PersistedStorage::KeyValueStoreMgrImpl().Get(
        key.c_str(),
        &matter_endpoint)
      == CHIP_NO_ERROR) {
...
{
  std::string keys
    = std::string(key.unify_unid) + "-" + std::to_string(key.unify_endpoint);
  chip::DeviceLayer::PersistedStorage::KeyValueStoreMgrImpl().Delete(
    keys.c_str());
}

// remove the persisted group mapping
template<> void matter_data_storage::remove_persisted_data(group_mapping &key)
{
