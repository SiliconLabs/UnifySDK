...
***NOTE*** To make UPTI CAP work, Silicon Labs WSTK adapters needs a special PTI enabled firmware

## Description

The _UPTI_CAP_ is an application to communicate with Silicon Labs WSTK
...
On startup, _UPTI_CAP_ automatically discovers and publish lists of debug adapters 
that are connected to the local subnet. You can also configure _UPTI_CAP_ to discover 
debug adapters on other subnets by providing the IP adressess list as the command line argument.
Adapters discovery can be started by publishing empty JSON to `ucl/UPTIСap/Discover` topic.
...
and publishes list of supported devices to `ucl/UPTIСap/List` topic.
...
messages and publishes trace packages to `ucl/UPTIСap/<SerialNumber>/TracePackage` topic.
...
publish control message to `ucl/UPTIСap/<SerialNumber>/Toggle` topic. 
...
* `ucl/UPTIСap/Discover` - topic to control PTI compatible devices discovery. Message payload - empty JSON.
```json
{
}
```

* `ucl/UPTIСap/List` - topic with the list of PTI compatible devices found in the network. Application publish list as retained message. 
```json
{
...
* `ucl/UPTIСap/<SerialNumber>/Toggle` - topic to control PTI coompatible device, where `SerialNumber` is the serial number of the WSTK board.
