                                 ...
                                 zwave_cluster_fan_mode fan_mode)
{
  ...
  if (call_type == UIC_MQTT_DOTDOT_CALLBACK_TYPE_SUPPORT_CHECK) {
    ...
    if (is_automatic_deduction_of_supported_commands_enabled()) {
      return (dotdot_is_supported_fan_control_z_wave_fan_mode(unid, endpoint))
               ? SL_STATUS_OK
               : SL_STATUS_FAIL;
    } else {
  ...
  if (is_desired_value_update_on_commands_enabled()) {
    ...
    sl_status_t result
      = dotdot_set_fan_control_z_wave_fan_mode(unid,
                                               endpoint,
                                               DESIRED_ATTRIBUTE,
                                               fan_mode);
    ...
    if (is_clear_reported_enabled()) {
      ...
      dotdot_fan_control_z_wave_fan_mode_undefine_reported(unid, endpoint);
    }
