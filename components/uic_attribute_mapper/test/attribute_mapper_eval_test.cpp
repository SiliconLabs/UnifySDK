...
ast::result_type eval_simple( std::string expr) {
  ...
  ast::eval eval(attribute::root(), ast);
...
{
  ast::ast_tree ast;
  ast::eval ass_evaluator(attribute::root(), ast);
  ast::attribute_path_eval path_evaluator(attribute::root(), ast);
  ...
  TEST_ASSERT_EQUAL(1234, path_evaluator(ass.attribute.attribute).type());
}
...
void test_decendant_path()
{
  ...
  ast::scope scope = boost::get<ast::scope>(ast[0]);
  ast::assignment ass = scope.assignments[0];
  TEST_ASSERT_EQUAL(5555, path_evaluator(ass.attribute.attribute).type());
}
...
void test_child_decendant_path()
{
  ...
  TEST_ASSERT_EQUAL(3333, path_evaluator(ass.attribute.attribute).type());
}

void test_dep_eval() {

  ast::ast_tree ast;
  ast::dep_eval dep_path_evaluator(ast);
  TEST_ASSERT_TRUE(ast::Parse("scope 0 { r'5555 = r'1234.42[0].3333 }", ast));
  ast::scope scope = boost::get<ast::scope>(ast[0]);
  ast::assignment ass = scope.assignments[0];

  ast::dep_eval eval(ast);
  eval(ass.rhs);

  const std::set<ast::attribute_dependency_t> &dependencies = eval.get_dependencies();
...
void test_simple_assignment()
{
  ...
  ast::attribute attr;
  attr.value_type = 'd';
  attr.attribute.push_back(ast::operand(6));  // <-- extra layer
