...
pub fn generate_extern_c(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let mut wrapper_function = parse_macro_input!(clone as syn::ItemFn);
    transmute_to_c_wrapper(&mut wrapper_function);
...
/// # Usage
/// This macro allows for attribute resolver callback functions to be implemented
/// as a pure rust functions with the signature:
///
/// ````ignore
///  fn my_resolver_get_callback(attr: Attribute) -> Result<(sl_status_t, Vec<u8>), AttributeStoreError>
/// ````
///
/// The functions should return a touple of sl_status and Vec<u8>. The sl_status_t is the normal
/// Resolver status Vec<u8> is the generated frame, if any. This the function is allowed to
/// Return a AttributeStoreError, if it does SL_STATUS_FAIL is sent back to the attribute resolver.
///
/// The function must still be registred using
/// ````ignore
/// attribute_resolver_register_rule(
///    MY_ATTRIBUTE_ID,
///    None,
///    as_extern_c!(my_resolver_get_callback),
///);
/// ````
///
#[proc_macro_attribute]
pub fn generate_resolver_callback(_attr: TokenStream, item: TokenStream) -> TokenStream {
    let mut func = parse_macro_input!(item as syn::ItemFn);
    let old_name = func.sig.ident;
    let new_name = quote::format_ident!("{}_rust", old_name);
    ...
    let output = quote! {
        ...
        fn #old_name(_node: u32, data: *mut u8, data_len: *mut u16) -> sl_status_t {
            ...
            match #new_name(node) {
                Ok( (status, frame) ) => {
                    unsafe {
                        std::ptr::copy(frame.as_ptr(), data, frame.len());
                        *data_len = frame.len() as u16;
                    }
                    status
                },
                Err(_) => {
...
/// macro to be used in combination with [macro@generate_extern_c]
/// This macro converts a given Rust function to the correct extern C function name.
/// See [macro@generate_extern_c] for more information.
#[proc_macro]
...
fn transmute_to_c_wrapper(func: &mut syn::ItemFn) {
    ...
    func.sig.unsafety = Some(Unsafe(Span::call_site()));
