...
pub enum AttributeStoreError {
    ...
    LegacyError(uic_attribute_store_bindings::sl_status_t),
    TypeMismatch {
        expected_size: usize,
        actual_size: usize,
    },
...
pub mod attribute_store {
    ...
    use uic_log_rs::{log_error, log_warning};
    use uic_proc_macro::{as_extern_c, generate_extern_c};

    use super::*;
    use crate::uic_attribute_store_bindings;

    const LOG_TAG: &str = "Attribute_store_rs";
    ...
    /// ```ignore
    /// use crate::uic_attribute_store_rs::{attribute_store_node_value_state_t::REPORTED_ATTRIBUTE, attribute_store::read_attribute};
    ...
    pub fn write_attribute<T: Serialize + Any>(
        ...
        let value_bytes =
            serialize_data(value).map_err(AttributeStoreError::SerializeError)?;
    ...
    fn deserialize_data<T: DeserializeOwned + Any>(
        ...
        type result_type<T> = Result<T, AttributeStoreError>;
        let mut res: result_type<T> = Err(AttributeStoreError::SerializeError(Box::new(
        ...
        if let Some(val) = (&mut res as &mut dyn Any).downcast_mut::<result_type<String>>() {
        ...
        } else if let Some(val) = (&mut res as &mut dyn Any).downcast_mut::<result_type<Vec<u8>>>()
        {
            *val = Ok(data);
        } else {
    ...
    type timeout_callback_box =
        Box<dyn FnOnce(uic_attribute_store_bindings::attribute_store_node_t)>;
    ...
    static mut PENDING_TIMEOUT_CALLBACKS: OnceCell<
        MultiMap<
            uic_attribute_store_bindings::attribute_store_node_t,
            (std::time::SystemTime, timeout_callback_box),
        >,
    > = OnceCell::new();
            ...
            if let Some(callbacks) = PENDING_TIMEOUT_CALLBACKS.get_mut() {
                if let Some(functions) = callbacks.get_vec_mut(&node) {
                    ...
                    if let Some(item) = functions.pop() {
                        ...
                        log_warning(
                        LOG_TAG,
                        format!("expected an callback to be present in the cache but it isnt! nodeid={}",
                        node),
                    );
                    }
                } else {
                    log_error(
                        LOG_TAG,
                        format!("Internal timeout callback cache error! nodeid={}", node),
                    );
                }
            } else {
                log_error(
    ...
    where
        F: 'static + FnOnce(uic_attribute_store_bindings::attribute_store_node_t),
    {
        unsafe {
            ...
            let duration_seconds: u64 =
                duration.as_secs() * uic_attribute_store_bindings::CLOCK_SECOND as u64;
            ...
            let res = uic_attribute_store_bindings::attribute_timeout_set_callback(
                node,
    ...
    pub fn attribute_store_get_node_attribute_value(
        node: attribute_store_node_t,
        value_state: attribute_store_node_value_state_t,
    ) -> Result<Vec<u8>, uic_attribute_store_bindings::sl_status_t> {
        unsafe {
