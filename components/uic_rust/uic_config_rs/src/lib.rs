...
//         non_upper_case_globals
//     )]
//     include!(concat!(env!("LIBUIC_DIR"), "/", "uic_config_bindings.rs"));
// }
mod uic_config_bindings;
pub use uic_config_bindings::*;
...
pub fn config_add_string(name: &str, help: &str, value: &str) -> Result<(), config_status_t> {
    ...
    let status = unsafe {
        uic_config_bindings::config_add_string(c_name.as_ptr(), c_help.as_ptr(), c_value.as_ptr())
    };
...
pub fn config_add_int(name: &str, help: &str, value: i32) -> Result<(), config_status_t> {
    ...
    let status =
        unsafe { uic_config_bindings::config_add_int(c_name.as_ptr(), c_help.as_ptr(), c_value) };
    convert_to_rust_result((), status)
}
...
pub fn config_add_double(name: &str, help: &str, value: f64) -> Result<(), config_status_t> {
    ...
    let status = unsafe {
        uic_config_bindings::config_add_double(c_name.as_ptr(), c_help.as_ptr(), c_value)
    };
    convert_to_rust_result((), status)
}
...
pub fn config_add_bool(name: &str, help: &str, value: bool) -> Result<(), config_status_t> {
    ...
    let status =
        unsafe { uic_config_bindings::config_add_bool(c_name.as_ptr(), c_help.as_ptr(), c_value) };
    convert_to_rust_result((), status)
}
...
pub fn config_parse(args: Vec<CString>, version: &str) -> Result<(), config_status_t> {
    ...
    let status = unsafe {
        uic_config_bindings::config_parse(
            c_args.len() as c_int,
...
pub fn config_get_as_string(name: &str) -> Result<String, config_status_t> {
    ...
    unsafe {
        let status = uic_config_bindings::config_get_as_string(c_name.as_ptr(), &mut str_value);
        match status {
...
pub fn config_get_as_int(name: &str) -> Result<i32, config_status_t> {
    ...
    let status = unsafe { uic_config_bindings::config_get_as_int(c_name.as_ptr(), &mut value) };
...
pub fn config_get_as_double(name: &str) -> Result<f64, config_status_t> {
    ...
    let status = unsafe { uic_config_bindings::config_get_as_double(c_name.as_ptr(), &mut value) };
    convert_to_rust_result(value, status)
}
...
pub fn config_get_as_bool(name: &str) -> Result<bool, config_status_t> {
    ...
    let status = unsafe { uic_config_bindings::config_get_as_bool(c_name.as_ptr(), &mut value) };
    convert_to_rust_result(value, status)
}

pub fn config_reset() {
    unsafe {
        uic_config_bindings::config_reset();
    }
}
