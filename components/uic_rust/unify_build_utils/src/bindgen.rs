...
///```no_run
///let includes = [".", "bar"];
///generate_bindings("foo.rs", &includes, None, None, &["foo.h"])?
///```
///
...
/// generate_bindings(&out_file, [], Some("foo"), None, Some("codebase/component/foo.h"))?
///```
///
...
pub fn generate_bindings<P: AsRef<Path>>(
    ...
    println!(
        "cargo:warning= generating binding file {}",
    ...
    let expanded = expand_headers(headers.unwrap_or_default())?;
