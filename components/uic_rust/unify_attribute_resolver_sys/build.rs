...
/// When building in the monorepo we use paths relative to the CARGO_MANIFEST_DIR.
/// When building for packages we use an absolute path to the libunify install output.
fn bindings() -> Result<()> {
    let include = load_environment("uic_attribute_resolver")?.include_directories;
    let binding_file =
    ...
    generate_bindings(
        &binding_file,
        &include,
        Some(".*attribute_resolver.*|.*is_node_pending_set_resolution.*"),
        None,
        &[header],
    )
}
