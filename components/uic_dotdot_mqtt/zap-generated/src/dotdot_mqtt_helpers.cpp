...
// Enum to string map for ZWaveHumidityControlCurrentState
const std::map<uint32_t, std::string> z_wave_humidity_control_current_state_enum_id_to_string_map {
  { 0, "Idle" },
  { 1, "Humidifying" },
  { 2, "Dehumidifying" },
};

// String to enum map for ZWaveHumidityControlCurrentState
const std::map<std::string, uint32_t> z_wave_humidity_control_current_state_enum_string_to_id_map {
  { "Idle", 0 },
  { "Humidifying", 1 },
  { "Dehumidifying", 2 },
};
...
std::string z_wave_humidity_control_current_state_get_enum_value_name(
  uint32_t value)
{
  auto it = z_wave_humidity_control_current_state_enum_id_to_string_map.find(value);
  if (it != z_wave_humidity_control_current_state_enum_id_to_string_map.end()){
    return it->second;
  }
...
{
  auto it = z_wave_humidity_control_current_state_enum_string_to_id_map.find(str);
  if (it != z_wave_humidity_control_current_state_enum_string_to_id_map.end()){
    return it->second;
  }
  ...
  uint32_t value)
{
  ...
  if (64928 == cluster_id) {
  #ifdef Z_WAVE_HUMIDITY_CONTROL_REPORTING_MODE_ENUM_NAME_AVAILABLE
    if (1 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlreporting_mode instead of this: z_wave_humidity_control_reporting_mode
      return z_wave_humidity_control_reporting_mode_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_SUPPORTED_REPORTING_MODE_ENUM_NAME_AVAILABLE
    if (2 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlsupported_reporting_mode instead of this: z_wave_humidity_control_supported_reporting_mode
      return z_wave_humidity_control_supported_reporting_mode_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_CURRENT_STATE_ENUM_NAME_AVAILABLE
    if (3 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlcurrent_state instead of this: z_wave_humidity_control_current_state
      return z_wave_humidity_control_current_state_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_SUPPORTED_SET_POINTS_ENUM_NAME_AVAILABLE
    if (4 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlsupported_set_points instead of this: z_wave_humidity_control_supported_set_points
      return z_wave_humidity_control_supported_set_points_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_MIN_ENUM_NAME_AVAILABLE
    if (5 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint_min instead of this: z_wave_humidity_control_humidifier_setpoint_min
      return z_wave_humidity_control_humidifier_setpoint_min_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_MAX_ENUM_NAME_AVAILABLE
    if (6 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint_max instead of this: z_wave_humidity_control_humidifier_setpoint_max
      return z_wave_humidity_control_humidifier_setpoint_max_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_ENUM_NAME_AVAILABLE
    if (7 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint instead of this: z_wave_humidity_control_humidifier_setpoint
      return z_wave_humidity_control_humidifier_setpoint_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_SCALE_ENUM_NAME_AVAILABLE
    if (8 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint_scale instead of this: z_wave_humidity_control_humidifier_setpoint_scale
      return z_wave_humidity_control_humidifier_setpoint_scale_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_PRECISION_ENUM_NAME_AVAILABLE
    if (9 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint_precision instead of this: z_wave_humidity_control_humidifier_setpoint_precision
      return z_wave_humidity_control_humidifier_setpoint_precision_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_MIN_ENUM_NAME_AVAILABLE
    if (10 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint_min instead of this: z_wave_humidity_control_dehumidifier_setpoint_min
      return z_wave_humidity_control_dehumidifier_setpoint_min_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_MAX_ENUM_NAME_AVAILABLE
    if (11 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint_max instead of this: z_wave_humidity_control_dehumidifier_setpoint_max
      return z_wave_humidity_control_dehumidifier_setpoint_max_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_ENUM_NAME_AVAILABLE
    if (12 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint instead of this: z_wave_humidity_control_dehumidifier_setpoint
      return z_wave_humidity_control_dehumidifier_setpoint_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_SCALE_ENUM_NAME_AVAILABLE
    if (13 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint_scale instead of this: z_wave_humidity_control_dehumidifier_setpoint_scale
      return z_wave_humidity_control_dehumidifier_setpoint_scale_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_PRECISION_ENUM_NAME_AVAILABLE
    if (14 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint_precision instead of this: z_wave_humidity_control_dehumidifier_setpoint_precision
      return z_wave_humidity_control_dehumidifier_setpoint_precision_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_MIN_ENUM_NAME_AVAILABLE
    if (15 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint_min instead of this: z_wave_humidity_control_auto_setpoint_min
      return z_wave_humidity_control_auto_setpoint_min_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_MAX_ENUM_NAME_AVAILABLE
    if (16 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint_max instead of this: z_wave_humidity_control_auto_setpoint_max
      return z_wave_humidity_control_auto_setpoint_max_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_ENUM_NAME_AVAILABLE
    if (17 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint instead of this: z_wave_humidity_control_auto_setpoint
      return z_wave_humidity_control_auto_setpoint_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_SCALE_ENUM_NAME_AVAILABLE
    if (18 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint_scale instead of this: z_wave_humidity_control_auto_setpoint_scale
      return z_wave_humidity_control_auto_setpoint_scale_get_enum_value_name(value);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_PRECISION_ENUM_NAME_AVAILABLE
    if (19 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint_precision instead of this: z_wave_humidity_control_auto_setpoint_precision
      return z_wave_humidity_control_auto_setpoint_precision_get_enum_value_name(value);
    }
  #endif
  }
  ...
  const std::string &name)
{
  ...
  if (64928 == cluster_id) {
    ...
    if (1 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlreporting_mode instead of this: z_wave_humidity_control_reporting_mode
      return z_wave_humidity_control_reporting_mode_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_SUPPORTED_REPORTING_MODE_ENUM_NAME_AVAILABLE
    if (2 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlsupported_reporting_mode instead of this: z_wave_humidity_control_supported_reporting_mode
      return z_wave_humidity_control_supported_reporting_mode_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_CURRENT_STATE_ENUM_NAME_AVAILABLE
    if (3 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlcurrent_state instead of this: z_wave_humidity_control_current_state
      return z_wave_humidity_control_current_state_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_SUPPORTED_SET_POINTS_ENUM_NAME_AVAILABLE
    if (4 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlsupported_set_points instead of this: z_wave_humidity_control_supported_set_points
      return z_wave_humidity_control_supported_set_points_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_MIN_ENUM_NAME_AVAILABLE
    if (5 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint_min instead of this: z_wave_humidity_control_humidifier_setpoint_min
      return z_wave_humidity_control_humidifier_setpoint_min_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_MAX_ENUM_NAME_AVAILABLE
    if (6 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint_max instead of this: z_wave_humidity_control_humidifier_setpoint_max
      return z_wave_humidity_control_humidifier_setpoint_max_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_ENUM_NAME_AVAILABLE
    if (7 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint instead of this: z_wave_humidity_control_humidifier_setpoint
      return z_wave_humidity_control_humidifier_setpoint_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_SCALE_ENUM_NAME_AVAILABLE
    if (8 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint_scale instead of this: z_wave_humidity_control_humidifier_setpoint_scale
      return z_wave_humidity_control_humidifier_setpoint_scale_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_HUMIDIFIER_SETPOINT_PRECISION_ENUM_NAME_AVAILABLE
    if (9 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlhumidifier_setpoint_precision instead of this: z_wave_humidity_control_humidifier_setpoint_precision
      return z_wave_humidity_control_humidifier_setpoint_precision_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_MIN_ENUM_NAME_AVAILABLE
    if (10 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint_min instead of this: z_wave_humidity_control_dehumidifier_setpoint_min
      return z_wave_humidity_control_dehumidifier_setpoint_min_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_MAX_ENUM_NAME_AVAILABLE
    if (11 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint_max instead of this: z_wave_humidity_control_dehumidifier_setpoint_max
      return z_wave_humidity_control_dehumidifier_setpoint_max_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_ENUM_NAME_AVAILABLE
    if (12 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint instead of this: z_wave_humidity_control_dehumidifier_setpoint
      return z_wave_humidity_control_dehumidifier_setpoint_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_SCALE_ENUM_NAME_AVAILABLE
    if (13 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint_scale instead of this: z_wave_humidity_control_dehumidifier_setpoint_scale
      return z_wave_humidity_control_dehumidifier_setpoint_scale_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_DEHUMIDIFIER_SETPOINT_PRECISION_ENUM_NAME_AVAILABLE
    if (14 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controldehumidifier_setpoint_precision instead of this: z_wave_humidity_control_dehumidifier_setpoint_precision
      return z_wave_humidity_control_dehumidifier_setpoint_precision_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_MIN_ENUM_NAME_AVAILABLE
    if (15 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint_min instead of this: z_wave_humidity_control_auto_setpoint_min
      return z_wave_humidity_control_auto_setpoint_min_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_MAX_ENUM_NAME_AVAILABLE
    if (16 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint_max instead of this: z_wave_humidity_control_auto_setpoint_max
      return z_wave_humidity_control_auto_setpoint_max_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_ENUM_NAME_AVAILABLE
    if (17 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint instead of this: z_wave_humidity_control_auto_setpoint
      return z_wave_humidity_control_auto_setpoint_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_SCALE_ENUM_NAME_AVAILABLE
    if (18 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint_scale instead of this: z_wave_humidity_control_auto_setpoint_scale
      return z_wave_humidity_control_auto_setpoint_scale_get_enum_value_number(name);
    }
  #endif
  #ifdef Z_WAVE_HUMIDITY_CONTROL_AUTO_SETPOINT_PRECISION_ENUM_NAME_AVAILABLE
    if (19 == attribute_id) {
      // FIXME: Some attributes don't work because multi-upper case names end up like this: z_wave_humidity_controlauto_setpoint_precision instead of this: z_wave_humidity_control_auto_setpoint_precision
      return z_wave_humidity_control_auto_setpoint_precision_get_enum_value_number(name);
    }
  #endif
  }
...
char *z_wave_humidity_control_current_state_get_enum_value_name_c(
  uint32_t value, char *result, size_t max_result_size)
{
  snprintf(result, max_result_size, "%s", z_wave_humidity_control_current_state_get_enum_value_name(value).c_str());
  return result;
}

uint32_t z_wave_humidity_control_current_state_get_enum_value_number_c(const char *str)
{
  return z_wave_humidity_control_current_state_get_enum_value_number(std::string(str));
}
