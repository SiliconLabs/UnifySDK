...
std::string get_json_payload_for_ota_upgrade_image_notify_command(
  ...
  command_with_no_fields = false;
  // Single Value
  // Enum ImageNotify / PayloadType
  #ifdef IMAGE_NOTIFY_PAYLOAD_TYPE_ENUM_NAME_AVAILABLE
  // Pick up the name from the value.
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["QueryJitter"] = nlohmann::json(fields->query_jitter);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["ImageType"] = nlohmann::json(fields->image_type);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["NewFileVersion"] = nlohmann::json(fields->new_file_version);
...
void uic_mqtt_dotdot_parse_ota_upgrade_image_notify(
  nlohmann::json &jsn,
  ...
  uint32_t PayloadType_enum_val = get_enum_decimal_value<ImageNotifyPayloadType>("PayloadType", jsn);
  if (PayloadType_enum_val == std::numeric_limits<ImageNotifyPayloadType>::max()) {
    #ifdef IMAGE_NOTIFY_PAYLOAD_TYPE_ENUM_NAME_AVAILABLE
    PayloadType_enum_val = image_notify_payload_type_get_enum_value_number(jsn.at("PayloadType").get<std::string>());
    #endif
  }
  if (jsn.at("PayloadType").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
  payload_type = static_cast<ImageNotifyPayloadType>(PayloadType_enum_val);
  if (jsn.at("QueryJitter").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  query_jitter = jsn.at("QueryJitter").get< uint8_t >();
  ...
  manufacturer_code = jsn.at("ManufacturerCode").get< uint16_t >();
      if (jsn.at("ImageType").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  image_type = jsn.at("ImageType").get< uint16_t >();
      if (jsn.at("NewFileVersion").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  new_file_version = jsn.at("NewFileVersion").get< uint32_t >();
    }


std::string get_json_payload_for_ota_upgrade_query_next_image_request_command(
  ...
  // Single Value
  // Known bitmask, pull the list of "names" from the value
  json_payload["FieldControl"] = uic_dotdot_mqtt::get_bitmap_value_names(
                                (uint32_t)fields->field_control,
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["CurrentFileVersion"] = nlohmann::json(fields->current_file_version);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["HardwareVersion"] = nlohmann::json(fields->hardware_version);
...
void uic_mqtt_dotdot_parse_ota_upgrade_query_next_image_request(
  nlohmann::json &jsn,
  ...
  if (jsn.at("FieldControl").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
  field_control = (uint8_t)uic_dotdot_mqtt::get_bitmap_decimal_value("FieldControl", jsn, QueryNextImageRequestFieldControl);
  if (jsn.at("ManufacturerCode").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
  ...
  image_type = jsn.at("ImageType").get< OTAImageType >();
      if (jsn.at("CurrentFileVersion").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  current_file_version = jsn.at("CurrentFileVersion").get< uint32_t >();
      if (jsn.at("HardwareVersion").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  hardware_version = jsn.at("HardwareVersion").get< uint16_t >();
    }


std::string get_json_payload_for_ota_upgrade_query_next_image_response_command(
  ...
  command_with_no_fields = false;
  // Single Value
  // Enum QueryNextImageResponse / Status
  #ifdef QUERY_NEXT_IMAGE_RESPONSE_STATUS_ENUM_NAME_AVAILABLE
  // Pick up the name from the value.
  json_payload["Status"] =
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["FileVersion"] = nlohmann::json(fields->file_version);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["ImageSize"] = nlohmann::json(fields->image_size);
...
void uic_mqtt_dotdot_parse_ota_upgrade_query_next_image_response(
  nlohmann::json &jsn,
  zclStatus &status,
  ...
  image_type = jsn.at("ImageType").get< OTAImageType >();
      if (jsn.at("FileVersion").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  file_version = jsn.at("FileVersion").get< uint32_t >();
      if (jsn.at("ImageSize").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  image_size = jsn.at("ImageSize").get< uint32_t >();
    }


std::string get_json_payload_for_ota_upgrade_image_block_request_command(
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["FileOffset"] = nlohmann::json(fields->file_offset);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["MaximumDataSize"] = nlohmann::json(fields->maximum_data_size);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["RequestNodeAddress"] = nlohmann::json(fields->request_node_address);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["MinimumBlockPeriod"] = nlohmann::json(fields->minimum_block_period);
...
void uic_mqtt_dotdot_parse_ota_upgrade_image_block_request(
  nlohmann::json &jsn,
  uint8_t &field_control,
  ...
  field_control = (uint8_t)uic_dotdot_mqtt::get_bitmap_decimal_value("FieldControl", jsn, ImageBlockRequestFieldControl);
  if (jsn.at("ManufacturerCode").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
  ...
  file_version = jsn.at("FileVersion").get< uint32_t >();
      if (jsn.at("FileOffset").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  file_offset = jsn.at("FileOffset").get< uint32_t >();
      if (jsn.at("MaximumDataSize").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  maximum_data_size = jsn.at("MaximumDataSize").get< uint8_t >();
      if (jsn.at("RequestNodeAddress").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  request_node_address = jsn.at("RequestNodeAddress").get< EUI64 >();
      if (jsn.at("MinimumBlockPeriod").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  minimum_block_period = jsn.at("MinimumBlockPeriod").get< uint16_t >();
    }


std::string get_json_payload_for_ota_upgrade_image_page_request_command(
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["PageSize"] = nlohmann::json(fields->page_size);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["ResponseSpacing"] = nlohmann::json(fields->response_spacing);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["RequestNodeAddress"] = nlohmann::json(fields->request_node_address);
...
void uic_mqtt_dotdot_parse_ota_upgrade_image_page_request(
  nlohmann::json &jsn,
  uint8_t &field_control,
  ...
  field_control = (uint8_t)uic_dotdot_mqtt::get_bitmap_decimal_value("FieldControl", jsn, ImagePageRequestFieldControl);
  if (jsn.at("ManufacturerCode").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
  ...
  maximum_data_size = jsn.at("MaximumDataSize").get< uint8_t >();
      if (jsn.at("PageSize").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  page_size = jsn.at("PageSize").get< uint16_t >();
      if (jsn.at("ResponseSpacing").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  response_spacing = jsn.at("ResponseSpacing").get< uint16_t >();
      if (jsn.at("RequestNodeAddress").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
...
std::string get_json_payload_for_ota_upgrade_image_block_response_command(
  ...
  command_with_no_fields = false;
  // Single Value
  // Enum ImageBlockResponse / Status
  #ifdef IMAGE_BLOCK_RESPONSE_STATUS_ENUM_NAME_AVAILABLE
  // Pick up the name from the value.
  json_payload["Status"] =
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["ImageData"] = nlohmann::json(fields->image_data);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["CurrentTime"] = nlohmann::json(fields->current_time);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["RequestTime"] = nlohmann::json(fields->request_time);
  command_with_no_fields = false;
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["MinimumBlockPeriod"] = nlohmann::json(fields->minimum_block_period);
...
void uic_mqtt_dotdot_parse_ota_upgrade_image_block_response(
  nlohmann::json &jsn,
  zclStatus &status,
  ...
  current_time = jsn.at("CurrentTime").get< uint32_t >();
      if (jsn.at("RequestTime").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  request_time = jsn.at("RequestTime").get< uint32_t >();
      if (jsn.at("MinimumBlockPeriod").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
...
std::string get_json_payload_for_ota_upgrade_upgrade_end_request_command(
  ...
  command_with_no_fields = false;
  // Single Value
  // Enum UpgradeEndRequest / Status
  #ifdef UPGRADE_END_REQUEST_STATUS_ENUM_NAME_AVAILABLE
  // Pick up the name from the value.
  json_payload["Status"] =
...
void uic_mqtt_dotdot_parse_ota_upgrade_upgrade_end_request(
  nlohmann::json &jsn,
  zclStatus &status,
...
std::string get_json_payload_for_ota_upgrade_upgrade_end_response_command(
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["UpgradeTime"] = nlohmann::json(fields->upgrade_time);
...
void uic_mqtt_dotdot_parse_ota_upgrade_upgrade_end_response(
  nlohmann::json &jsn,
  uint16_t &manufacturer_code,
  ...
  current_time = jsn.at("CurrentTime").get< UTC >();
      if (jsn.at("UpgradeTime").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  upgrade_time = jsn.at("UpgradeTime").get< UTC >();
    }


std::string get_json_payload_for_ota_upgrade_query_device_specific_file_request_command(
  ...
  command_with_no_fields = false;
  // Single Value
  // Enum QueryDeviceSpecificFileRequest / ImageType
  #ifdef QUERY_DEVICE_SPECIFIC_FILE_REQUEST_IMAGE_TYPE_ENUM_NAME_AVAILABLE
  // Pick up the name from the value.
  ...
  // Single Value
  // Non-enum and non-bitmask (struct, string or scalar)
  json_payload["CurrentZigbeeStackVersion"] = nlohmann::json(fields->current_zigbee_stack_version);
...
void uic_mqtt_dotdot_parse_ota_upgrade_query_device_specific_file_request(
  nlohmann::json &jsn,
  EUI64 &request_node_address,
  ...
  uint32_t ImageType_enum_val = get_enum_decimal_value<OTADeviceSpecificImageType>("ImageType", jsn);
  if (ImageType_enum_val == std::numeric_limits<OTADeviceSpecificImageType>::max()) {
    #ifdef OTA_DEVICE_SPECIFIC_IMAGE_TYPE_ENUM_NAME_AVAILABLE
    ImageType_enum_val = ota_device_specific_image_type_get_enum_value_number(jsn.at("ImageType").get<std::string>());
    #endif
  }
  if (jsn.at("RequestNodeAddress").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
  ...
  image_type = static_cast<OTADeviceSpecificImageType>(ImageType_enum_val);
  if (jsn.at("FileVersion").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
  ...
  file_version = jsn.at("FileVersion").get< uint32_t >();
      if (jsn.at("CurrentZigbeeStackVersion").is_null()) {
    sl_log_debug(LOG_TAG, "Ignoring JSON Null object");
    return;
  }
        
  current_zigbee_stack_version = jsn.at("CurrentZigbeeStackVersion").get< uint16_t >();
    }


std::string get_json_payload_for_ota_upgrade_query_device_specific_file_response_command(
  ...
  command_with_no_fields = false;
  // Single Value
  // Enum QueryDeviceSpecificFileResponse / Status
  #ifdef QUERY_DEVICE_SPECIFIC_FILE_RESPONSE_STATUS_ENUM_NAME_AVAILABLE
  // Pick up the name from the value.
  json_payload["Status"] =
  ...
  command_with_no_fields = false;
  // Single Value
  // Enum QueryDeviceSpecificFileResponse / ImageType
  #ifdef QUERY_DEVICE_SPECIFIC_FILE_RESPONSE_IMAGE_TYPE_ENUM_NAME_AVAILABLE
  // Pick up the name from the value.
  json_payload["ImageType"] =
...
void uic_mqtt_dotdot_parse_ota_upgrade_query_device_specific_file_response(
  nlohmann::json &jsn,
  zclStatus &status,
 ...
 * Parse incoming JSON object to populate command arguments passed in by reference.
 */
void uic_mqtt_dotdot_parse_ota_upgrade_write_attributes(
  nlohmann::json &jsn,
