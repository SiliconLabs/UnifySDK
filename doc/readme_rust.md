...
The Unify SDK has support for programming in Rust. Several components are already written using Rust and more components will be added in the future. This document describes how to setup, build and develop rust components for the Unify SDK.

> The docker images provided by the Unify SDK contain already everything you need to program Rust.
...
- ZPC binary, \<project_root\>/applications/zpc/components/zwave_rust
...
```bash
echo ". uic_root/my_build_folder/unify_env.source" >> ~/.bashrc
```
...
2. Dynamic loading. We created a script that scans for multiple build directories and asks you wich environment to load. You can execute it manually every time. or choose to add it to your shell profile. i.e.
 ...
 ```bash
echo "uic_root/scripts/source_env_prompt.sh" >> ~/.bashrc
```
...
please select the correct environment to source:
  [0]    /Users/svradema/git/uic/build
  [1]    /Users/svradema/git/uic/build_arm
0
sourced!
```
